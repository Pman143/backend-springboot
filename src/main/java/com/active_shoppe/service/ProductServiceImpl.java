...
public class ProductServiceImpl implements ProductService {
    ...
    public List<ProductDto> getAllProducts() {
        ...
        return productModels.stream().map(productModel -> ProductDto.builder()
    ...
    public ResponseDto buyProducts(String customerId, ProductsCodeDto productsCodeDto) {
        ...
        if (!customerModel.isPresent()) {
            throw new BusinessException(String.format("Customer with id [%s] does not exist.", customerId), HttpStatus.NOT_FOUND);
        }

        if (customerModel.get().getCustomerActiveDaysModel().getPoints() <= 0) {
            throw new BusinessException("You have insufficient points for making a purchase", HttpStatus.NOT_ACCEPTABLE);
        }

        if (productsCodeDto.getProductCode().isEmpty()) {
            throw new BusinessException("Please provide a valid product code", HttpStatus.BAD_REQUEST);
        }
